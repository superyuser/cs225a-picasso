...
SUPPORT_CLOSE_FRAC = 0.030      # relative to image width, closed silhouette helper
SUPPORT_DILATE_FRAC = 0.012
...
HAIR_FALLBACK_MIN_PRIOR_COVERAGE = 0.25
SHIRT_FALLBACK_MIN_PRIOR_COVERAGE = 0.25

# Hatching
HATCH_STEP_FACTOR = 0.60          # smaller = denser; 0.55-0.68 usually good
...
HATCH_SAFE_MARGIN_FACTOR = 0.42   # < 0.5 allows coverage in narrow hair strands
...
def generate_hatch_fill_strokes(mask, region_name, canvas_w_mm, canvas_h_mm, brush_width_mm, angle_deg,
    ...
    # Stable painterly order: across direction, not totally random.
    ...
    for off in offsets:
        ...
        cv2.line(line_img, tuple(np.round(p1).astype(int)), tuple(np.round(p2).astype(int)), 255, 1, cv2.LINE_AA)
